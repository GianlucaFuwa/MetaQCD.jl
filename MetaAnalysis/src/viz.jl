...
struct MetaMeasurements
    ...
    function MetaMeasurements(ensemblename::String; fullpath=false)
        dir = if fullpath
            ...
            ddir = chop(pkgdir(MetaAnalysis); tail=12)
            ddir * "/ensembles/$(ensemblename)/measurements/"
        end
        hmc_logfile = chop(pkgdir(MetaAnalysis); tail=12) * "/ensembles/$(ensemblename)/logs/hmc_acc_logs.txt"
...
struct MetaBias{F}
    ...
    function MetaBias(ensemblename::String; which=nothing, stream=1, fullpath=false)
        dir = if fullpath
            ...
            ddir = chop(pkgdir(MetaAnalysis); tail=12)
            ddir * "/ensembles/$(ensemblename)/metapotentials/"
        end
        @assert isdir(dir) "Directory \"$(dir)\" doesn't exist."
        filenames = readdir(dir)
        ...
        T = typeof(bias)
        whichin = if T === Metadynamics
            "MetaD"
        elseif T === OPES
            "OPES"
        end
        return new{T}(bias, whichin, ensemblename)
    end
end
