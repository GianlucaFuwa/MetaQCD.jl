...
macro level1(msg)
    ...
    return quote
        if __GlobalLogger[].LEVEL ≥ 1 && mpi_amroot(mpi_comm_instance())
...
macro level2(msg)
    pmsg = prepare_message(msg)
    return quote
        if __GlobalLogger[].LEVEL ≥ 2 && mpi_amroot(mpi_comm_instance())
            __GlobalLogger[].to_console && printf($pmsg)
            !isnothing(__GlobalLogger[].fp) && printf(__GlobalLogger[].fp, $pmsg)
        end
        nothing
...
macro level3(msg)
    pmsg = prepare_message(msg)
    return quote
        if __GlobalLogger[].LEVEL ≥ 3 && mpi_amroot(mpi_comm_instance())
            __GlobalLogger[].to_console && printf($pmsg)
            !isnothing(__GlobalLogger[].fp) && printf(__GlobalLogger[].fp, $pmsg)
        end
        nothing
...
macro level4(msg)
    pmsg = prepare_message(msg)
    return quote
        if __GlobalLogger[].LEVEL ≥ 4 && mpi_amroot(mpi_comm_instance())
            __GlobalLogger[].to_console && printf($pmsg)
            !isnothing(__GlobalLogger[].fp) && printf(__GlobalLogger[].fp, $pmsg)
        end
        nothing
