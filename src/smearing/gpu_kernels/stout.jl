function apply_stout_smearing!(
    ...
    @latmap(Sequential(), Val(1), apply_stout_smearing_gpu!, Uout, C, Q, U, T(ρ), bulk)
    return nothing
end

@kernel function apply_stout_smearing_gpu!(Uout, C, Q, @Const(U), ρ, bulk)
...
function stout_recursion!(
    ...
    @latmap(Sequential(), Val(1), stout_recursion_gpu!, Σ, Σ′, U, C, Q, Λ, T(ρ), bulk)
    return nothing
end

@kernel function stout_recursion_gpu!(
...
function calc_stout_Λ!(
    ...
    @latmap(Sequential(), Val(1), calc_stout_Λ_gpu!, Λ, Σ′, Q, U, bulk)
    return nothing
end

@kernel function calc_stout_Λ_gpu!(Λ, @Const(Σ′), @Const(Q), @Const(U), bulk)
    iglobal = @index(Global, Cartesian)
    site = bulk[iglobal]
