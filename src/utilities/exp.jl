...
function exp_iQ(Q::SMatrix{3,3,ComplexF64,9})
    u, w, c0max, signflip = set_uw(Q)
    f0, f1, f2, _ = set_fj(u, w, c0max, signflip)
...
function calc_coefficients(Q::SMatrix{3,3,ComplexF64,9})
    u, w, c0max, signflip = set_uw(Q)
    f0, f1, f2, ξ0 = set_fj(u, w, c0max, signflip)
    e2iu = cis(2u)
    emiu = cis(-u)
    cosw = cos(w)
    w2 = w^2
...
function set_uw(Q::SMatrix{3,3,ComplexF64,9})
    ...
    u = c13r * cos(Θ/3)
    w = sqrt(c1) * sin(Θ/3)
    return u, w, c0max, signflip
end

function set_fj(u, w, c0max, signflip)
    w2 = w^2
    u2 = u^2
    #if abs(w) <= 0.05
    #    w2 = w2
