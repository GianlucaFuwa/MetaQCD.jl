...
function calc_dSfdU!(dU, fermion_action::WilsonFermionAction, U, ψ::WilsonFermionfield)
    ...
    @assert fermion_action.D.csw === nothing "Force calculation for clover Wilson not supported"
...
function TA_from_XY!(dU, U, X::T, Y::T, anti) where {T<:WilsonFermionfield}
    ...
    for site in eachindex(dU)
        ...
        dU[4, site] = monehalf * traceless_antihermitian(cmatmul_oo(U[4, site], B - C))
    end
end
