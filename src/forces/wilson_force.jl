function calc_dSfdU!(
    ...
    LinearAlgebra.mul!(temps[1], D, X) # Need to prefix with LinearAlgebra to avoid ambiguity with Gaugefields.mul!
    add_wilson_derivative!(dU, U, X, temps[1], bc)

    if has_clover_term(D)
        Xμν = fermion_action.Xμν
        calc_Xμν_wilson_eachsite!(Xμν, X, temps[1])
...
function calc_dSfdU!(
    ...
    for i in 1:n
        ...
        if has_clover_term(D)
            Xμν = fermion_action.Xμν
            calc_Xμν_wilson_eachsite!(Xμν, Xs[i+1], temps[1])
