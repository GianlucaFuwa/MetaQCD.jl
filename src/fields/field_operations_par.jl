...
function LinearAlgebra.norm(u::AbstractField{CPU}, ::Val{2}) # avg 2-norm
    ...
    @batch reduction=(+, norm2) for μsite in allindices(u)
        norm2 += cnorm2(u[μsite])
    end
...
function LinearAlgebra.norm(u::AbstractField{CPU}, ::Val{Inf}) # FIXME: max of 2-norms, Inf misleading
    ...
    @batch reduction=(max, normsup) for μsite in allindices(u)
        normsup = max(normsup, cnorm2(u[μsite])) 
    end
