function parallelfor(f, itr, ::Type{backend}, args...; block_size::Int=256) where {backend}
    if backend == CPU
...
function _foreachindex_gpu(f, indices, backend::GPU; block_size::Int=256)
    # GPU implementation
    @assert block_size > 0
    blocks = (length(indices) + block_size - 1) ÷ block_size
    _foreachindex_global!(backend, block_size)(f, indices, ndrange=(block_size * blocks,))
    return nothing
end
...
function parallelfor_sum(
    f, itr, init, ::Type{backend}, args...; block_size::Int=256
) where {backend}
    if backend == CPU
        result = init
...
function _foreachindex_reduce_gpu(
    out, op, f, indices, ::Type{backend}; block_size::Int=256
) where {backend}
    # GPU implementation
    @assert block_size > 0
    blocks = (length(indices) + block_size - 1) ÷ block_size
