...
function update_halo_gpu_multi!(
    ...
    launch_mpi_calls!(reqs, sendbufs, streams, partitioned_dims, fields...)
...
function update_halo_gpu_multi_edges!(
    ...
    for dim in partitioned_dims
        ...
        launch_mpi_calls!(reqs, sendbufs, streams, (dim,), fields...)
...
function launch_mpi_calls!(
    ...
    # Post all receives first so transfers can start as soon as sends are posted.
    for (idim, dim) in enumerate(dims)
    ...
    # Post sends as soon as each direction stream has finished packing.
    for (idim, dim) in enumerate(dims)
        nbrs = mpi_cart_shift(comm_cart, dim-1, 1)
        for inbr in 1:2
            ...
            for ifield in 1:N
                ...
                tags = 8ifield + (2*(dim-1) + 1), 8ifield + (2*(dim-1) + 2)
                send_req = mpi_isend(
                    sendbufs[idx, idim], comm_cart; dest=nbrs[inbr], tag=tags[mod1(inbr+1, 2)]
                )
            ...
            for inbr in 1:2
                for i in 1:N
                    ...
                    if !sendrecv_ready[recv_idx] && reqs[recv_idx] != Utils.MPI.REQUEST_NULL
                        if Utils.MPI.Test(reqs[recv_idx])
                            u = fields[i]
                            recv_buf = get_recv_buf(u, 2*(dim-1) + inbr)
                    ...
                    if !sendrecv_ready[send_idx] && reqs[send_idx] != Utils.MPI.REQUEST_NULL
                        send_done = Utils.MPI.Test(reqs[send_idx])
                        if send_done
                            sendrecv_ready[send_idx] = true
                            n_pending -= 1
                            any_progress = true
                        end
                    end
