...
# function __latsum(
#     ::Checkerboard2, ::Val{COUNT}, ::Type{OutType}, f!::F, U::AbstractField{B}, args...
# ) where {COUNT,OutType,F,B<:GPU}
#     COUNT == 0 && return 0.0
#     NX, NY, NZ, NT = local_dims(U)
#     @assert(
...
#
# function __latsum(
#     ::Checkerboard4, ::Val{COUNT}, ::Type{OutType}, f!::F, U::AbstractField{B}, args...
# ) where {COUNT,OutType,F,B<:GPU}
#     COUNT == 0 && return 0.0
#     NX, NY, NZ, NT = local_dims(U)
#     @assert(
...
function __latsum(
    ::EvenSites, ::Val{COUNT}, ::Type{OutType}, f!::F, U::AbstractField{B}, args...
) where {COUNT,OutType,F,B<:GPU}
    COUNT == 0 && return 0.0
    fdims = local_dims(U)
    ndrange = ntuple(i -> (i == 4 ? fdims[i]÷2 : fdims[i]), Val(4))
    workgroupsize = ntuple(i -> min(ndrange[i], 4), Val(4))
    numblocks = cld(U.NV, prod(workgroupsize))
    out = KA.zeros(B(), OutType, numblocks)
    kernel! = f!(B(), workgroupsize)
