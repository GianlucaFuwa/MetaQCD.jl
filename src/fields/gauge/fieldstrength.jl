        ...
        for iz in 1:NZ
            for iy in 1:NY
                for ix in 1:NX
                    ...
                    F[1][2][ix,iy,iz,it] = im * traceless_antihermitian(C12)

                    C13 = plaquette(U, 1, 3, site)
                    F[1][3][ix,iy,iz,it] = im * traceless_antihermitian(C13)
                    
                    C14 = plaquette(U, 1, 4, site)
                    F[1][4][ix,iy,iz,it] = im * traceless_antihermitian(C14)
                    
                    C23 = plaquette(U, 2, 3, site)
                    F[2][3][ix,iy,iz,it] = im * traceless_antihermitian(C23)

                    C24 = plaquette(U, 2, 4, site)
                    F[2][4][ix,iy,iz,it] = im * traceless_antihermitian(C24)

                    C34 = plaquette(U, 3, 4, site)
                    F[3][4][ix,iy,iz,it] = im * traceless_antihermitian(C34)
                end
            end
        end
    end
