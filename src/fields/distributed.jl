...
function Utils.update_halo!(u::AbstractMPIField)
    ...
    for dim in 1:4
        ...
        if prev_neighbor == next_neighbor == mpi_myrank(comm_instance)
            ...
            mpi_isend(send_buf_next, comm_cart, requests[2]; dest=next_neighbor, tag=1)
            mpi_irecv!(recv_buf_next, comm_cart, requests[3]; source=next_neighbor, tag=2)
            mpi_isend(send_buf_prev, comm_cart, requests[4]; dest=prev_neighbor, tag=2)
